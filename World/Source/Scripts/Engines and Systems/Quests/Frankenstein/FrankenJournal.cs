...
namespace Server.Items
{
	public class FrankenJournal : Item
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			{
				from.SendMessage( StringCatalog.Resolve( from.Account, "This journal does not belong to you toss it out!" ) );
				bool remove = true;
		...
		private class FrankenGump : Gump
		{
			...
			public FrankenGump( FrankenJournal book, Mobile from ) : base( 50, 50 )
			{
				...
				{
					AddItem(12, 430, 9698);
					AddHtml( 55, 430, 261, 20, StringCatalog.ResolveFormat( from.Account, "<BODY><BASEFONT Color={0}>From {1}</BASEFONT></BODY>", color, book.BrainFrom ), (bool)false, (bool)false);
					AddHtml( 55, 460, 261, 20, StringCatalog.ResolveFormat( from.Account, "<BODY><BASEFONT Color={0}>Level {1} Brain</BASEFONT></BODY>", color, book.HasBrain ), (bool)false, (bool)false);
				}
				...
				if ( bodyParts > 6 )
				{
					...
					AddHtml( 55, 535, 261, 20, StringCatalog.ResolveFormat( from.Account, "<BODY><BASEFONT Color={0}>Reanimate a Slave</BASEFONT></BODY>", color ), (bool)false, (bool)false);

					AddButton(12, 565, 4005, 4005, 2, GumpButtonType.Reply, 0);
					AddHtml( 55, 565, 261, 20, StringCatalog.ResolveFormat( from.Account, "<BODY><BASEFONT Color={0}>Reanimate a Protector</BASEFONT></BODY>", color ), (bool)false, (bool)false);
				}
			}
			...
			public override void OnResponse( NetState sender, RelayInfo info )
			{
				if ( info.ButtonID > 0 )
				{
					...
					if ( nearCoil )
					{
						...
						m_From.PrivateOverheadMessage(MessageType.Regular, 1153, false, StringCatalog.Resolve( m_From.Account, "My experiment is a success." ), m_From.NetState);
					...
					{
						m_From.SendMessage( StringCatalog.Resolve( m_From.Account, "You need to be near a power coil to do that." ) );
