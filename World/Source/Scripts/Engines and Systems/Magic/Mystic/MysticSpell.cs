...
namespace Server.Spells.Mystic
{
	public abstract class MysticSpell : Spell
	{
		...
		{
			string txt = StringCatalog.Resolve( account, "This parchment holds the knowledge of the mystics: " );
			string skl = "0";

			if ( spell == 250 ){ 			skl = "80";	txt += StringCatalog.Resolve( account, "Enter the astral plane where your soul is immune to harm. While you are in this state, you can freely travel but your interraction with the world is minimal. The better your skill, the longer it lasts. Monks use this ability to safely travel through dangerous areas." ); }
			else if ( spell == 251 ){ 		skl = "50";	txt += StringCatalog.Resolve( account, "Travel through the astral plane to another location with the use of a magical recall rune. The rune must be marked by other magical means before you can travel to that location. If you wish to travel using a rune book, then set your rune book's default location and then you can target the book while using this ability." ); }
			else if ( spell == 252 ){ 		skl = "25";	txt += StringCatalog.Resolve( account, "Creates a robe that you will need in order to use the other abilities in this tome. The robe will have power based on your overall skill as a monk, and no one else may wear the robe. You can only have one such robe at a time, so creating a new robe will cause any others you own to go back to the astral plane. After creation, single click the robe and select the 'Enchant' option to spend the points on attributes you want the robe to have." ); }
			else if ( spell == 253 ){ 		skl = "30";	txt += StringCatalog.Resolve( account, "Perform a soothing touch, healing damage sustained. The higher your skill, the more damage you will heal with your touch." ); }
			else if ( spell == 254 ){ 		skl = "35";	txt += StringCatalog.Resolve( account, "Allows you to leap over a long distance. This is a quick action and can allow a monk to leap toward an opponent, leap away to safety, or leap over some obstacles like rivers and streams." ); }
			else if ( spell == 255 ){ 		skl = "30";	txt += StringCatalog.Resolve( account, "Summon your Ki to perform a mental attack that deals an amount of energy damage based upon your fist fighting and intelligence values. Elemental Resistances may reduce damage done by this attack." ); }
			else if ( spell == 256 ){ 		skl = "60";	txt += StringCatalog.Resolve( account, "You sheer force of will creates a barrier around you, deflecting magical attacks. This does not work against odd magics like necromancy. Affected spells will often bounce back onto the caster." ); }
			else if ( spell == 257 ){ 		skl = "40";	txt += StringCatalog.Resolve( account, "You can cleanse your body of poisons with this ability due to your physical discipline, and as such, it cannot be used to aid anyone else." ); }
			else if ( spell == 258 ){ 		skl = "20";	txt += StringCatalog.Resolve( account, "You must be wearing some sort of pugilist gloves for this ability to work. It temporarily enhances the kind of damage the gloves do. The type of damage inflicted when hitting a target will be converted to the target's worst resistance type. The duration of the effect is affected by your fist fighting skill." ); }
			else if ( spell == 259 )
			{
				skl = "70";	txt += StringCatalog.Resolve( account, "This ability allows the monk to run as fast as a steed. This ability should be avoided if you already have a mount you are riding, or perhaps you have magical boots that allow you to run at this speed. using this ability in such conditions may cause unusual travel speeds, so be leery." );
				if ( MySettings.S_NoMountsInCertainRegions )
					txt += StringCatalog.Resolve( account, " Be aware when exploring the land, that there are some areas you cannot use this ability in. These are areas such as dungeons, caves, and some indoor areas. If you enter such an area, this ability will be hindered." );
			}

			if ( skl == "0" )
				return txt;

			return txt + " " + StringCatalog.ResolveFormat( account, "It requires a Mystic to be at least a {0} in ability to use it.", skl );
		}
		...
		public override bool CheckCast()
		{
			...
			{
				Caster.SendMessage( StringCatalog.Resolve( Caster.Account, "Your equipment or skills are not commensurate to that of a true monk." ) );
				return false;
			}
