	...
	public class IceThrenodySong : Song
	{
		...
		public void Target(Mobile m)
		{
			...
			else if (CheckHSequence(m))
			{
				...
				var recipient = new IceThrenodyRecipient(m, duration, amount);
		...
		private class IceThrenodyRecipient : TimeDependentRecipient<IceThrenodySong>
		{
			private readonly int m_Amount;
			private ResistanceMod m_Mod;

			public IceThrenodyRecipient(Mobile targetMobile, TimeSpan duration, int amount) : base(targetMobile, duration)
			{
				m_Amount = amount;
			}
			...
			protected override bool TryApplyInternal()
			{
				...
				m.SendMessage( StringCatalog.Resolve( m.Account, "Your resistance to cold has decreased." ) );
				m_Mod = new ResistanceMod(ResistanceType.Cold, -m_Amount);
				m.AddResistanceMod(m_Mod);
				m.FixedParticles(0x374A, 10, 30, 5013, 0x480, 2, EffectLayer.Waist);

				string args = String.Format("{0}", m_Amount);
				BuffInfo.RemoveBuff(m, BuffIcon.IceThrenody);
				BuffInfo.AddBuff(m, new BuffInfo(BuffIcon.IceThrenody, 1063575, 1063576, Duration, m, args, true));
