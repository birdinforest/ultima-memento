...
namespace Server.Mobiles
{
	...
	public class TitanPyros : BaseCreature
	{
		...
		public override bool OnBeforeDeath()
		{
				...
				if ( m is PlayerMobile && m.Map == this.Map && !m.Blessed )
				{
					...
					if ( obelisk != null )
					{
						...
						if ( tip.ObeliskOwner == m && tip.HasFire > 0 && tip.WonFire < 1 )
						{
							...
							winner = m;
							tip.WonFire = 1;
							m.SendMessage( StringCatalog.ResolveByKey(m.Account, "mob.other.you_absord_the_titan_s_power_into_the_tongue_of_flame") );
							m.PlaySound( 0x65A );
			...
			{
				Say(StringCatalog.ResolveByKey(this.Account, "mob.other.no_it_is_your_soul_i_will_take"));
				...
				if ( winner != null )
				{
					...
					if ( winner is PlayerMobile && !winner.Blessed )
					{
						...
						if ( p != null )
						{
							...
							{
								if ( pmi.Mobile is PlayerMobile && pmi.Mobile.InRange(this.Location, 20) && pmi.Mobile.Map == this.Map && !pmi.Mobile.Blessed && !Server.Misc.PlayerSettings.GetSpecialsKilled( pmi.Mobile, StringCatalog.ResolveByKey(this.Account, "mob.other.titanpyros") ) )
								{
									Server.Misc.PlayerSettings.SetSpecialsKilled( pmi.Mobile, StringCatalog.ResolveByKey(this.Account, "mob.other.titanpyros"), true );
						...
						else if ( !Server.Misc.PlayerSettings.GetSpecialsKilled( winner, StringCatalog.ResolveByKey(this.Account, "mob.other.titanpyros") ) )
						{
							Server.Misc.PlayerSettings.SetSpecialsKilled( winner, StringCatalog.ResolveByKey(this.Account, "mob.other.titanpyros"), true );
							ManualOfItems book = new ManualOfItems();
								book.Hue = RewardColor;
								book.ItemID = 0x1AA3;
								book.Name = "Chest of Fire Titan Relics";
