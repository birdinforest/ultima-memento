...
namespace Server.Mobiles
{
	...
	public class TitanStratos : BaseCreature
	{
		...
		public override bool OnBeforeDeath()
		{
				...
				if ( m is PlayerMobile && m.Map == this.Map && !m.Blessed )
				{
					...
					if ( obelisk != null )
					{
						...
						if ( tip.ObeliskOwner == m && tip.HasAir > 0 && tip.WonAir < 1 )
						{
							...
							winner = m;
							tip.WonAir = 1;
							m.SendMessage( StringCatalog.ResolveByKey(m.Account, "mob.other.you_absord_the_titan_s_power_into_the_breath_of_air") );
							m.PlaySound( 0x65A );
			...
			{
				Say(StringCatalog.ResolveByKey(this.Account, "mob.other.no_the_power_of_the_wind_will_blow_you_away"));
