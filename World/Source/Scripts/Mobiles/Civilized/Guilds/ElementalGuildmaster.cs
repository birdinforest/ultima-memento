        ...
        private class RepairTarget : Target
        {
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                if (targeted is HenchmanFamiliarItem && from.Backpack != null)
                {
                    ...
                    {
						m_Elementalist.SayTo(from, Server.Localization.StringCatalog.Resolve(from.Account, "That crystal ball has too many charges already."));
                    }
                    ...
                    if (pack.ConsumeTotal(typeof(Gold), toConsume))
                    {
                        ...
                        m_Elementalist.SayTo(from, Server.Localization.StringCatalog.Resolve(from.Account, "Your crystal ball is charged."));
                    ...
                    {
                        m_Elementalist.SayTo(from, Server.Localization.StringCatalog.ResolveFormat(from.Account, "It would cost you {0} gold to have that charged.", toConsume));
                        from.SendMessage(Server.Localization.StringCatalog.Resolve(from.Account, "You do not have enough gold."));
                    }
                }
				else
				{
					m_Elementalist.SayTo(from, Server.Localization.StringCatalog.Resolve(from.Account, "That does not need my services."));
				}
            }
        }
