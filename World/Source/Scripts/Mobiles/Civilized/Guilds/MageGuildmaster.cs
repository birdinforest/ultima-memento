        ...
        private class RepairTarget : Target
        {
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                if (targeted is HenchmanFamiliarItem && from.Backpack != null)
                {
                    ...
                    {
						m_Mage.SayTo(from, Server.Localization.StringCatalog.ResolveByKey(from.Account, "mob.other.that_crystal_ball_has_too_many_charges_already"));
                    }
                    ...
                    if (pack.ConsumeTotal(typeof(Gold), toConsume))
                    {
                        ...
                        m_Mage.SayTo(from, Server.Localization.StringCatalog.ResolveByKey(from.Account, "mob.other.your_crystal_ball_is_charged"));
                    ...
                    {
                        m_Mage.SayTo(from, Server.Localization.StringCatalog.ResolveFormatByKey(from.Account, "mob.fmt.it_would_cost_you_0_gold_to_have_that_charged", toConsume));
                        from.SendMessage(Server.Localization.StringCatalog.ResolveByKey(from.Account, "mob.other.you_do_not_have_enough_gold"));
                    }
                }
				else
				{
					m_Mage.SayTo(from, Server.Localization.StringCatalog.ResolveByKey(from.Account, "mob.other.that_does_not_need_my_services"));
				}
            }
        }
