...
namespace Server.Items
{
	public class HenchmanItem : Item
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			{
				from.SendMessage(StringCatalog.ResolveByKey(from.Account, "mob.other.they_are_dead_and_you_must_hire_a_healer_to_resurrect_t"));
			}
			...
			else if ( HenchmanFunctions.IsInRestRegion( from ) == true )
			{
				...
				if ( !(this is HenchmanMonsterItem) )
				{
					...
					{
						case 0: friend.Say(StringCatalog.ResolveByKey(friend.Account, "mob.other.i_am_ready_for_adventure")); break;
						case 1: friend.Say(StringCatalog.ResolveByKey(friend.Account, "mob.other.where_are_we_off_to")); break;
						case 2: friend.Say("" + from.Name + ", I am here to serve."); break;
						case 3: friend.Say(StringCatalog.ResolveByKey(friend.Account, "mob.other.may_monsters_fear_our_hardy_band")); break;
						case 4: friend.Say(StringCatalog.ResolveByKey(friend.Account, "mob.other.what_dungeon_do_we_dare_explore")); break;
						case 5: friend.Say(StringCatalog.ResolveByKey(friend.Account, "mob.other.i_have_been_waiting_here_too_long")); break;
						case 6: friend.Say(StringCatalog.ResolveByKey(friend.Account, "mob.other.hello") + from.Name + "."); break;
						case 7: friend.Say(StringCatalog.ResolveByKey(friend.Account, "mob.other.fame_and_fortune_await")); break;
					}
				}
			}
			else
			{
				from.SendMessage(StringCatalog.ResolveByKey(from.Account, "mob.other.you_can_only_call_your_henchman_from_a_home_inn_or_tave"));
			}
		}
		...
		public static void ResurrectHenchman( Mobile from )
		{
			if ( from is PlayerMobile && from.Alive )
			{
				...
				if ( giveMessage )
				{
					from.SendMessage(StringCatalog.ResolveByKey(from.Account, "mob.other.your_henchmen_have_been_resurrected"));
