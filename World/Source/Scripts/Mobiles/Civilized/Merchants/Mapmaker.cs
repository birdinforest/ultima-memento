        ...
        private class RepairTarget : Target
        {
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                ...
                if (targeted is TreasureMap && from.Backpack != null)
                {
					...
					{
						m_Mapmaker.SayTo(from, Server.Localization.StringCatalog.ResolveByKey(from.Account, "mob.other.that_map_has_already_been_deciphered"));
					}
                    else if (pack.ConsumeTotal(typeof(Gold), toConsume))
                    {
						...
						if ( tmap.Level == 1 ){ m_Mapmaker.SayTo(from, Server.Localization.StringCatalog.ResolveByKey(from.Account, "mob.other.this_map_was_really_quite_simple")); }
						else if ( tmap.Level == 2 ){ m_Mapmaker.SayTo(from, Server.Localization.StringCatalog.ResolveByKey(from.Account, "mob.other.seemed_pretty_easy_so_here_it_is")); }
						else if ( tmap.Level == 3 ){ m_Mapmaker.SayTo(from, Server.Localization.StringCatalog.ResolveByKey(from.Account, "mob.other.this_map_was_a_bit_of_a_challenge")); }
						else if ( tmap.Level == 4 ){ m_Mapmaker.SayTo(from, Server.Localization.StringCatalog.ResolveByKey(from.Account, "mob.other.whoever_drew_this_map_did_not_want_it_found")); }
						else if ( tmap.Level == 5 ){ m_Mapmaker.SayTo(from, Server.Localization.StringCatalog.ResolveByKey(from.Account, "mob.other.this_took_more_research_than_normal")); }
						else { m_Mapmaker.SayTo(from, Server.Localization.StringCatalog.ResolveByKey(from.Account, "mob.other.with_the_ancient_writings_and_riddles_this_map_should_n")); }
                    ...
                    {
                        m_Mapmaker.SayTo(from, Server.Localization.StringCatalog.ResolveFormatByKey(from.Account, "mob.fmt.it_would_cost_you_0_gold_for_me_to_decipher_that_map", toConsume));
                        from.SendMessage(Server.Localization.StringCatalog.ResolveByKey(from.Account, "mob.other.you_do_not_have_enough_gold"));
                    }
                }
				else
				{
					m_Mapmaker.SayTo(from, Server.Localization.StringCatalog.ResolveByKey(from.Account, "mob.other.that_does_not_need_my_services"));
				}
            }
        }
