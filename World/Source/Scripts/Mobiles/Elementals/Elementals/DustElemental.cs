		...
		public override void OnGaveMeleeAttack( Mobile m )
		{
			...
			if ( 1 == Utility.RandomMinMax( 1, 20 ) )
			{
				...
				if ( iSucked != null )
				{
					...
					{
						m.LocalOverheadMessage(MessageType.Emote, 1150, true, Server.Localization.StringCatalog.ResolveByKey(m.Account, "mob.other.one_of_your_protected_items_was_almost_carried_by_the_w"));
					}
					else
					{
						m.LocalOverheadMessage(MessageType.Emote, 0x916, true, Server.Localization.StringCatalog.ResolveByKey(m.Account, "mob.other.one_of_your_items_was_carried_into_the_wind"));
						m.PlaySound( 0x10B );
						PackItem( iSucked );
					}
				}
