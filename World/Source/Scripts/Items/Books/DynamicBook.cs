...
namespace Server.Items
{
	public class DynamicBook : Item
	{
		...
		public static string ResolveBookBodyForViewer( Mobile viewer, DynamicBook book )
		{
			...
			if ( book is WorkShoppes )
				return ResolveWorkShoppesHtmlForViewer( viewer );

			if ( book is BookGuideToAdventure || book is LoreGuidetoAdventure )
				return ResolveGuideToAdventureHtmlForViewer( viewer );

			if ( book is RuneJournal )
				return ResolveRuneJournalHtmlForViewer( viewer );
		...
		private static void AppendRuneJournalSpellEntries( StringBuilder sb, TextInfo cultInfo )
		{
			...
			while ( run )
			{
				...
				if ( SpellItems.GetRunes( magicspell ) != "" )
					sb.Append( "<BR><BR>" ).Append( cultInfo.ToTitleCase( SpellItems.GetName( magicspell ) ) ).Append( "<BR>  " ).Append( SpellItems.GetRunes( magicspell ) ).Append( "<BR><BR>" ).Append( SpellItems.GetData( magicspell ) ).Append( "<BR>________________________" );
		...
		private static string ComposeBasicHelpEnglish()
		{
			string body = string.Format( BasicHelpBookText.Part1Format, MySettings.S_ServerName );

			if ( MySettings.S_Bribery >= 1000 )
				body += string.Format( BasicHelpBookText.BriberyFormat, MySettings.S_Bribery );

			body += BasicHelpBookText.Part3Suffix;
			return body;
		}

		/// <summary>Localized Guide to Adventure / Lore guide body.</summary>
		public static string ResolveGuideToAdventureHtmlForViewer( Mobile viewer )
		{
			string lang = ViewerLang( viewer );
			string p1Fmt = StringCatalog.TryResolve( lang, BasicHelpBookText.Part1Format ) ?? BasicHelpBookText.Part1Format;
			string body = string.Format( p1Fmt, MySettings.S_ServerName );

			if ( MySettings.S_Bribery >= 1000 )
			{
				string bFmt = StringCatalog.TryResolve( lang, BasicHelpBookText.BriberyFormat ) ?? BasicHelpBookText.BriberyFormat;
				body += string.Format( bFmt, MySettings.S_Bribery );
			}

			body += StringCatalog.TryResolve( lang, BasicHelpBookText.Part3Suffix ) ?? BasicHelpBookText.Part3Suffix;
			return body;
		}

		/// <summary>Localized Rune Journal: fixed sections from catalog; spell lines from SpellItems.</summary>
		public static string ResolveRuneJournalHtmlForViewer( Mobile viewer )
		{
			string lang = ViewerLang( viewer );
			TextInfo cultInfo = new CultureInfo( "en-US", false ).TextInfo;
			StringBuilder sb = new StringBuilder();
			sb.Append( StringCatalog.TryResolve( lang, RuneJournalBookText.IntroEnglish ) ?? RuneJournalBookText.IntroEnglish );
			sb.Append( StringCatalog.TryResolve( lang, RuneJournalBookText.FollowResearchEnglish ) ?? RuneJournalBookText.FollowResearchEnglish );
			sb.Append( StringCatalog.TryResolve( lang, RuneJournalBookText.RuneBagsSectionEnglish ) ?? RuneJournalBookText.RuneBagsSectionEnglish );
			sb.Append( StringCatalog.TryResolve( lang, RuneJournalBookText.RuneMeaningsEnglish ) ?? RuneJournalBookText.RuneMeaningsEnglish );
			sb.Append( StringCatalog.TryResolve( lang, RuneJournalBookText.SpellListHeaderEnglish ) ?? RuneJournalBookText.SpellListHeaderEnglish );
			AppendRuneJournalSpellEntries( sb, cultInfo );
			sb.Append( StringCatalog.TryResolve( lang, RuneJournalBookText.ClosingSpacersEnglish ) ?? RuneJournalBookText.ClosingSpacersEnglish );
			return sb.ToString();
		}
		...
		{
			return ComposeBasicHelpEnglish();
		}
		...
		public static void SetStaticText( DynamicBook book )
		{
			...
			{
				book.BookText = WorkShoppesBookText.BodyEnglish + ( MySettings.S_MerchantCrates ? WorkShoppesBookText.MerchantCrateSuffixEnglish : "" );
			}
			...
			else if ( book is RuneJournal )
			{
				TextInfo cultInfo = new CultureInfo( "en-US", false ).TextInfo;
				StringBuilder sb = new StringBuilder();
				sb.Append( RuneJournalBookText.IntroEnglish );
				sb.Append( RuneJournalBookText.FollowResearchEnglish );
				sb.Append( RuneJournalBookText.RuneBagsSectionEnglish );
				sb.Append( RuneJournalBookText.RuneMeaningsEnglish );
				sb.Append( RuneJournalBookText.SpellListHeaderEnglish );
				AppendRuneJournalSpellEntries( sb, cultInfo );
				sb.Append( RuneJournalBookText.ClosingSpacersEnglish );
				book.BookText = sb.ToString();
			}
