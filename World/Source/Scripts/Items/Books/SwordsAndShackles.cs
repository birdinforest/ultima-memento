	...
	public class SwordsAndShackles : Item
	{
		...
		public class SwordsAndShacklesGump : Gump
		{
			...
			private static string ResolveText( Mobile from, string text )
			{
				...
				string s = StringCatalog.TryResolve( lang, text ) ?? text;
				return QuestCompositeResolver.ResolveComposite( from, s );
			}

			private static string HtmlCenter( string color, string text )
			{
				return @"<BODY><BASEFONT Color=" + color + "><CENTER>" + text + "</CENTER></BASEFONT></BODY>";
			}

			private static string HtmlBody( string color, string text )
			{
