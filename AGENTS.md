...
| `some-random-note.json` | `SomeRandomNote` quest parchment templates: lie/truth variants (`quest.note.lie.*`, `quest.note.truth.*`) plus localizable poison, skull, document, and researcher vocabulary. Templates retain ordered `{n}` runtime arguments; the author-facing argument map lives in `tools-output/some-random-note-arg-spec.json`. Pair `en/` + `zh-Hans/`; **`keep_extra`**. |
| `inscription-recipe-drop.json` | Inscription advanced recipe (5th–8th circle scroll) boss-drop announcements (`sys.inscription.recipe_drop.*`). Pair `en/` + `zh-Hans/`; **`keep_extra`**. |
| `lore-books.json` | `LoreBook` title / author / body shotkeys (`lore.book.<slug>.title|author|body`) plus `lore.book.city.*` for `GetRandomCity()` template slots. `The Orb of the Abyss` uses `{0}` `{1}`; `The Underworld Gate` uses `{0}`. C# identity (`BookTitle` / `Name` / `BookAuthor`) stays English; view-time resolve is `LoreBook.ResolveLocalized*` via `DynamicBook`. Pair `en/` + `zh-Hans/`; **`keep_extra`**. Generator: `World/Source/Tools/build_lore_books_shotkeys.py` (reuses existing ZH; do not re-translate). |
...
- 2026-07-18: §8.1 — indexed `THIEF_MARKET_SYSTEM.md`（`Market.Thief` 盗贼市场：`Thief`/`ThiefGuildmaster`、开锁、`ThiefNote`）。
